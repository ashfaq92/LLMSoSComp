...
export class McpServer {
    ...
    createMcpServerInstance() {
        return new SdkMcpServer({ name: this.config.name, version: this.config.version }, {
            capabilities: {
                resources: {
                    subscribe: true,
                    listChanged: true
    ...
    registerGenericTools(server) {
        ...
        // Tool: read_property
        server.registerTool("read_property", {
            ...
            try {
                ...
                if (!prop)
                    throw new Error(`Property '${args.property_name}' not found on device '${args.device_id}'.`);
        ...
        // Tool: write_property
        server.registerTool("write_property", {
            ...
            try {
                ...
                if (!prop)
                    throw new Error(`Property '${args.property_name}' not found on device '${args.device_id}'.`);
                if (!prop.writable)
                    throw new Error(`Property '${args.property_name}' is read-only.`);
    ...
    applyThingToAllServers(thing) {
        const servers = [];
        if (this.stdioServer)
            servers.push(this.stdioServer);
    ...
    registerPropertyGetterToolOnServer(server, prop) {
    ...
    registerEventResourceOnServer(server, event) {
        server.registerResource(event.name, event.uri, {
    ...
    registerActionToolOnServer(server, action) {
    ...
    registerPropertySetterToolOnServer(server, prop) {
        // Parse URI to extract thingId and propertyName
        // URI format: wot://<thingId>/properties/<propertyName>
        const match = prop.uri.match(/^wot:\/\/([^/]+)\/properties\/(.+)$/);
        let toolName;
     ...
     */
    async handleWotEvent(thingId, eventName, data) {
     ...
     */
    async start(mode = 'stdio', port = 3000) {
        if (mode === 'stdio') {
    ...
    async invokeActionTool(action, args) {
    ...
    async invokePropertySetter(prop, args) {
